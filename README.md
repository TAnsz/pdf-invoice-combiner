...
[![Latest Release](https://img.shields.io/github/v/release/TAnsz/pdf-invoice-combiner)](https://github.com/TAnsz/pdf-invoice-combiner/releases)
...
- � **多文件管理**：支持选择多个文件、累积添加、双击删除、清空列表
- 🚀 **开箱即用**：提供独立 EXE 版本，无需安装 Python
- 🛡️ **错误恢复**：完善的错误处理，即使部分文件失败也能继续处理
...
2. **选择输入**：
   ...
   - 或选择"多个文件"模式：选择多个特定的 PDF 文件（可多次选择，文件会累积添加到列表中）
   - 使用"清空列表"按钮清空已选择的文件
   - 双击文件列表中的项目可删除该文件
...
### v1.1.0 (2026-04-07)
- 🎯 **GUI 增强**：支持多文件选择，可多次选择累积添加文件
- 🗑️ **清空功能**：添加"清空列表"按钮，一键清空所有选择的文件
- 👆 **双击删除**：文件列表支持双击删除单个文件
- 🛡️ **错误处理**：大幅改进合并过程中的错误处理和恢复机制
- 📝 **日志优化**：更详细的操作日志和错误信息显示
- 🔧 **稳定性**：即使部分页面处理失败，也会继续处理其他页面

### v1.0.1 (2026-04-05)
- 🎨 **图形界面**：新增完整的 GUI 版本，支持文件夹和文件选择
- 📊 **进度显示**：实时显示处理进度条和状态信息
- 📋 **文件列表**：显示选中的所有 PDF 文件
- 🔄 **多线程**：后台处理避免界面冻结
- 📦 **EXE 打包**：支持 PyInstaller 打包为独立可执行文件

### v1.0.0 (2026-04-03)
- ✨ **核心功能**：实现 PDF 页面智能合并算法
- ✂️ **自动裁剪**：智能检测并删除页面周围大白边
- 📏 **高度排序**：按页面高度排序，优化配对效果
- 🔍 **递归搜索**：自动搜索目录及子目录中的所有 PDF 文件
- 💾 **安全输出**：自动处理输出文件覆盖
- 📄 **命令行**：完整的命令行接口，支持参数配置

## 联系方式

如有问题或建议，请在 [Issues](https://github.com/TAnsz/pdf-invoice-combiner/issues) 中提出。
